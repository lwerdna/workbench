...
# notes:
# https://github.com/construct/construct/blob/master/deprecated_gallery/elf32.py

import os, sys, re
from construct import *

ELFCLASS = Enum(Int8ul,
    ELFCLASSNONE = 0,
    ELFCLASS32 = 1,
    ELFCLASS64 = 2
)
...
elf64_hdr = Struct(
    'e_ident[0..3]' / Const(b'\x7FELF'),
    'e_ident[EI_CLASS=4]' / ELFCLASS,
    'e_ident[EI_DATA=5]' / ELFDATA, 
    'e_ident[EI_VERSION=6]' / Int8ul,
    'e_ident[EI_OSABI=7]' / Int8ul,
    'e_ident[EI_ABIVERSION=8]' / Int8ul,
    'e_ident[EI_EI_PAD]' / Bytes(7),
    'e_type' / Int16ul,
    'e_machine' / Int16ul,
    'e_version' / Int32ul,
    'e_entry' / Int32ul,
...
def pprint_val(v):
    if type(v) == int:
        if v == 0: return 0
        return '0x%X (%d) (type:%s)' % (v,v,type(v))
    elif type == bytes:
        return v.hex()
    else:
        return str(v)

def pprint(con, values, depth=0):
    indent = '  '*depth

    if type(con) == Struct:
        for subcon in con.subcons:
            pprint(subcon, values, depth+1)
    elif type(con) == Renamed:
        print('%s(%s) %s: %s' % (indent, con.sizeof(), con.name, pprint_val(values[con.name])))
    else:
        print('cant handle type: %s' % type(con))
    

fpath = '/Users/andrewl/fdumps/filesamples/hello-linux-x64.elf'
with open(fpath, 'rb') as fp:
    buf = fp.read()

foo = elf64_hdr.parse(buf)
print(foo)
pprint(elf64_hdr, foo)
