...
int main(int ac, char *av[])
{
	...
	int status;
	long lret;
	uintptr_t addr, lo, hi, addr_main;
	...
	while (1) {
		...
		printf(".rax = 0x%" PRIxPTR "\n", regs.rax);
		printf(".orig_rax = 0x%" PRIxPTR "\n", regs.orig_rax);
		printf("system call %s\n", syscall_name(regs.orig_rax));

		if (regs.rip >= lo && regs.rip < hi)
			break;
	};
...
//		printf("address 0x%" PRIxPTR " is within module: %s\n", addr, module_name);
	...
	addr_main = addr + 0x1272;
	printf("injectee!main at 0x%" PRIxPTR "\n", addr_main);

	/* write memory */
	iret = write_mem(pid, addr_main, "\x90\x90\x90\x90\x90\x90\x0f\x05", 8); // nop, nop, ..., syscall
	if (iret != 0) {
	...
	rc = read_mem(pid, addr_main, DUMPSZ, buf);
	if (rc != 0) {
		exit(1);
	}
	hexdump(buf, DUMPSZ, addr_main);
	...
	/* did the setregs work? */
	addr = get_executing_address(pid);
	printf("verify setregs, executing address == 0x%" PRIxPTR "\n", addr);
