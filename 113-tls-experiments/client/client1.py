...
x509 = X509()
x509.parse(open('client_cert.pem').read())
certChain = X509CertChain([x509])

privateKey = parsePEMKey(open('client_key.pem').read(), private=True)

settings = HandshakeSettings()
settings.maxVersion = (3,3) # (3,0) is SSL 3.0, (3,1) is TLS 1.0
settings.minVersion = (3,3)
connection.handshakeClientCert(certChain=certChain, privateKey=privateKey, settings=settings)
