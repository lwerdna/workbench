...
# Demonstrate the general solution: the elements and relations form a graph, the transitive
# closure is a tree or a graph that can be culled into one, and the resulting tree's preorder
# traversal displays the hierarchy.
...
def traverse_preorder(G, v, depth=0):
    indent = '  '*depth

    print(indent + str(v))
    for child in sorted(G.successors(v), key=lambda x: get_interval_values(x)[0]):
        traverse_preorder(G, child, depth+1)

if __name__ == '__main__':
    n = len(items)
    print(f'// {n} items')
    print(f'// {n*(n+1)//2} relation members maximum')

    G = nx.DiGraph()

    for a in items:
        G.add_node(a)
        for b in [x for x in items if x != a]:
            G.add_node(b)
            if is_ancestor(a, b):
                G.add_edge(a, b)
                #print((a, b))

    print(f'// {len(G.edges)} relation members found by brute search')

    G = nx.transitive_reduction(G)

    print(f'// {len(G.edges)} relation members after transitive reduction')

    roots = [n for n in G.nodes if G.in_degree(n) == 0]
    assert len(roots) == 1
    root = roots[0]

    traverse_preorder(G, root)
