...
def calculate_strings(blob, width):
    assert width%2 == 0
    ...
    for i in range(len(blob)):
        ...
        n_ascii = len([b for b in sample if (b>ord(' ') and b<ord('~')) or b==0])
        result.append(n_ascii / width)
...
if __name__ == '__main__':
    ...
    entropy = calculate_strings(blob, width)
    ax2.set_xlabel('offset')
    ax2.set_ylabel('string score')
