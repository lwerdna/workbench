...
def create(code, data, code_addr):
    ...
    fields = {  'e_ident': b'\x7FELF',
                ...
                'e_version': 1,
                'e_entry': code_addr,
                'e_phoff': 0 + elf64_hdr_sz,
                'e_shoff': o_shdr_null,
                'e_flags': 0,
                'e_ehsize': elf64_hdr_sz, # elf header size
                'e_phentsize': elf64_phdr_sz, # program header size
                'e_phnum': n_program_headers,
                'e_shentsize': elf64_shdr_sz, # section header entry size
                'e_shnum': n_section_headers,
                'e_shstrndx': i_shdr_shstrtab # index of shdr that has the string table
    }

    elf64_hdr = struct.pack('<4sBBBBB7sHHIQQQIHHHHHH', *fields.values())
    assert len(elf64_hdr) == elf64_hdr_sz
    elf_data.seek(o_hdr)
    elf_data.write(elf64_hdr)
    ...
    fields = {  'p_type': 6, # PT_PHDR
                'p_flags': 5, # R-X
                'p_offset': 0 + elf64_hdr_sz, # location of program header
                'p_vaddr': imagebase + elf64_hdr_sz, # virtual address of program headers
                'p_paddr': imagebase + elf64_hdr_sz, # physical address program headers
                'p_filesz': n_program_headers * elf64_phdr_sz,
                'p_memsz': n_program_headers * elf64_phdr_sz,
                'p_align': 8
    }

    elf64_phdr0 = struct.pack('<IIQQQQQQ', *fields.values())
    assert len(elf64_phdr0) == elf64_phdr_sz
    elf_data.seek(o_phdr0)
    elf_data.write(elf64_phdr0)

    # SECOND PROGRAM HEADER LOADS THIS IMAGE TO MEM
    fields = {  'p_type': 1, # PT_LOAD
                'p_flags': 5, # R-X
                'p_offset': 0, # where in this file to start loading
                'p_vaddr': imagebase, # virtual address of program headers
                'p_paddr': imagebase, # physical address program headers
                'p_filesz': o_shstrtab,
                'p_memsz': align_up_to(o_shstrtab, 8),
                'p_align': 8
    }

    elf64_phdr1 = struct.pack('<IIQQQQQQ', *fields.values())
    assert len(elf64_phdr1) == elf64_phdr_sz
    elf_data.seek(o_phdr1)
    elf_data.write(elf64_phdr1)
    ...
    elf64_phdr2 = struct.pack('<IIQQQQQQ', *fields.values())
    assert len(elf64_phdr2) == elf64_phdr_sz
    elf_data.seek(o_phdr2)
    elf_data.write(elf64_phdr2)
    ...
    elf64_shdr0 = struct.pack('<IIQQQQIIQQ', *fields.values())
    assert len(elf64_shdr0) == elf64_shdr_sz
    elf_data.seek(o_shdr_null)
    elf_data.write(elf64_shdr0)
    ...
    elf64_shdr1 = struct.pack('<IIQQQQIIQQ', *fields.values())
    assert len(elf64_shdr1) == elf64_shdr_sz
    elf_data.seek(o_shdr_text)
    elf_data.write(elf64_shdr1)
    ...
    elf64_shdr2 = struct.pack('<IIQQQQIIQQ', *fields.values())
    assert len(elf64_shdr2) == elf64_shdr_sz
    elf_data.seek(o_shdr_data)
    elf_data.write(elf64_shdr2)

    # THIRD SECTION HEADER IS FOR .shstrtab
    fields = {  'sh_name': shstrtab.index(b'.shstrtab\x00'),
                'sh_type': 3, # STRTAB
                'sh_flags': 0,
                'sh_addr': 0, # doesn't need to be mapped
    ...
    elf64_shdr3 = struct.pack('<IIQQQQIIQQ', *fields.values())
    assert len(elf64_shdr3) == elf64_shdr_sz
    elf_data.seek(o_shdr_shstrtab)
    elf_data.write(elf64_shdr3)
