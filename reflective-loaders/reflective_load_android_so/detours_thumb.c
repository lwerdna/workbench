...
void *detour_mmap64(void *addr, size_t len, int prot, int flags, int fd, off64_t offset)
{
	...
	len_aligned = len;
	if(len & 0xFFF) 
		len_aligned = (len & 0xFFFFF000) + 0x1000;

	if(flags & MAP_FIXED) {
		int flip_write = !(prot & PROT_WRITE);

		if(flip_write)
			prot |= PROT_WRITE;

		prot_to_str(prot, prot_str);
		printf("new prot: 0x%X (%s)\n", prot, prot_str);
		tmp = mmap(addr, len, prot, flags | MAP_ANONYMOUS, -1, 0);
		if(tmp == MAP_FAILED) {
			printf("mmap() failed: %s\n", strerror(errno));
			goto cleanup;
		}
		printf("zzmmap() returned %p\n", tmp);
		...
		if(flip_write) {
			prot &= (~PROT_WRITE);
			if(mprotect(tmp, len_aligned, prot))
				ERR("mprotect()");
		}
	}
	else {
